...
Delta from v5: replaces ALIGNN's graph-level readout (`dgl.nn.AvgPooling`) with
`_Float64Pool`, a manual float64 mean (`torch.sum` / `index_add_`), casting the
pooled result back to the input dtype. Targets exactly `alignn_atomwise.py:466
-- h = self.readout(g, x)`.

What this fixes
---------------
The pre-cliff f32 pool-accumulation drift. On v5, `model((g, lg, lattice))["out"]`
...
in the message-passing layers switching algorithm at a size threshold. See
`analysis/v6/summary.md` for the full story and the two-drift explanation.

Validity window in f32: **N ≤ 442,368 (i=48)**. For N beyond that, neither the
pool wrapper nor any v5-era change is enough; cast the full model to float64
(OOMs above N ≈ 365k on 178 GiB) or bisect the message-passing layers.

Inherits from v5:
  - Sparse `_fast_line_graph` (the v5 bottleneck fix).
  - Checkpoint resume from the `.npz` on disk.
  - matscipy CPU neighbor list.

Usage
-----
    python scale5b_v6.py              # scaling sweep (resumes if .npz exists)
    python scale5b_v6.py --verify     # ASE + line-graph equivalence check
    python scale5b_v6.py --max-size N # cap size (default 99)
"""
...
# =============================================================================
# Float64 readout wrapper (the v6 correctness fix)
# =============================================================================

class _Float64Pool(nn.Module):
    """
    Drop-in replacement for DGL's AvgPooling that accumulates in float64 and
    casts back to the input dtype. Targets the graph-level readout at
    `alignn_atomwise.py:466 -- h = self.readout(g, x)`, which in float32 loses
    precision roughly as N * eps_f32 when summing ~10^6 node vectors.

    Why we do not simply call the wrapped `AvgPooling` with an upcast input:
    initial v6 attempt did exactly that and produced the same drift as v5
    (0.639 vs 0.636 at i=50, 0.845 vs 0.838 at i=58) -- empirically indicating
    DGL's segment-reduce kernel does not preserve float64 accumulation in
    practice on this build. Bypassing DGL and using `torch.sum` directly puts
    the reduction on a path where dtype is honored.

    Single-graph fast path (our benchmark): `feat64.sum(0) / N` with a
    keepdim=True to match AvgPooling's `[batch, hidden]` output shape.
    Batched fast path: loop-free segment sum via index_add_ keyed by the
    per-node graph id (built from `batch_num_nodes`).

    Cost: transient float64 copy of the input (at hidden_features=256,
    N=780k, that is ~1.6 GB -- well within HBM).
    """
    def __init__(self, base: nn.Module):
        super().__init__()
        # base is kept only as a sanity fallback; forward() never calls it.
        self.base = base
    ...
    def forward(self, graph, feat):
        ...
        if bnn.numel() == 1:
            pooled = feat64.sum(dim=0, keepdim=True) / bnn.to(feat64)
        else:
            graph_id = torch.repeat_interleave(
                torch.arange(bnn.numel(), device=feat64.device), bnn,
            )
            pooled = feat64.new_zeros((bnn.numel(), feat64.size(-1)))
            pooled.index_add_(0, graph_id, feat64)
            pooled = pooled / bnn.to(feat64).unsqueeze(-1)
        return pooled.to(feat.dtype)
...
def _load_alignn_ff(device: torch.device):
    ...
    # v6 correctness fix: float64 accumulation in the graph-level pool.
    model.readout = _Float64Pool(model.readout)
    if hasattr(model, "readout_feat"):
        model.readout_feat = _Float64Pool(model.readout_feat)
...
def run_scaling(
    ...
    verify: bool = True,
    checkpoint: str = "scaling_alignn_v6.npz",
    plot: str = "scaling_alignn_v6.png",
) -> None:
